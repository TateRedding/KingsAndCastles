...
public class Edit extends MapState implements StateMethods {
	...
	public void render(Graphics g) {
		...
		if (inGameArea) {
			drawSelectedTile(g);
			if (selectedTileType == CASTLE_ZONE)
	...
	private void drawSelectedTile(Graphics g) {
		if (selectedTileType != -1)
			g.drawImage(ImageLoader.editorBarTileButtonImages.get(selectedTileType), mouseX, mouseY, null);
		g.drawImage(ImageLoader.select, mouseX, mouseY, null);
	}

	private void drawPlayerIndicator(Graphics g) {
		g.setColor(Color.BLACK);
		g.setFont(LoadSave.getGameFont(Font.BOLD, 24f));
	...
	private void validateCastleZones() {
		...
		mapStatBar.setShowCastleZoneWarning(!valid);
	...
	public void mouseReleased(int x, int y, int button) {
		...
		else if (selectedTileType != -1 && inGameArea)
			if (leftMouseDown)
